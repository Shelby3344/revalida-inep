...
// Overlay de animação — vive no nível do App para sobreviver à troca de rota
function RevalidaOverlay() {
  return (
    <div className="revalida-overlay">
      <span className="revalida-overlay-text">
        Re<span className="revalida-overlay-v">v</span>alida
      </span>
    </div>
  );
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const id = window.requestAnimationFrame(() => root.classList.remove('theme-switching'));
  ...
  const handleEnter = useCallback(() => {
    setShowOverlay(true);
    // Navega enquanto overlay ainda está visível (cobre a troca de rota)
    setTimeout(() => navigate('/cadernos'), 1700);
    // Remove overlay depois que o fade-out termina
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f7] dark:bg-black text-gray-900 dark:text-[#f5f5f7] font-sans">
      ...
      {/* Overlay de transição (App-level) */}
      {showOverlay && <RevalidaOverlay />}

      {/* Header — oculto na landing */}
      {!isLanding && (
      ...
      <Suspense fallback={<PageFallback />}>
        <Routes>
          ...
          <Route path="/cadernos"    element={<ExamList />} />
          <Route path="/exam/:examId" element={<ExamViewer />} />
        </Routes>
      </Suspense>

      {/* Footer — oculto na landing */}
      {!isLanding && (
